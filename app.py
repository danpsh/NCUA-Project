...
def tearsheet_pdf(row, cycle, lens, hist, vals):
    ...
    el.append(Paragraph(
        "Source: NCUA 5300 Call Report. Composite score is a peer-relative percentile blend "
        "(50 = band median) under the selected lens. Loan and deposit composition are from "
    ...
    if not query:
        ...
        if not hits.empty:
            ...
            h[4].metric("Composite Score",
                        ...
                        help=f"{lens} band-percentile blend vs the asset-band peer group; "
                             "50 = band median.")
            st.markdown(
                f"<div style='font-size:1.6rem;line-height:1.1'>{stars_str(row.stars)}"
                "</div>", unsafe_allow_html=True)
            cap = f"{lens} score vs {row.band} peers — 50 = band median (percentile blend)."
